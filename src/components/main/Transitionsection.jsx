...
// 전체 SVG 컨테이너 scale 배율: 120→590
const CONTAINER_SCALE = 590 / 120; // ≈ 4.917
...
const TransitionSection = () => {
    ...
    useEffect(() => {
        ...
        if (!section || !bgEl || !leftImg || !rightImg || letters.length === 0) return;

        // 초기 상태
        letters.forEach((el, i) => {
            gsap.set(el, { scale: 0, transformOrigin: LETTER_ORIGINS[i], force3D: true });
        });
        ...
        const raf = requestAnimationFrame(() => {
            ...
            // ─── 핵심 최적화 ───────────────────────────────────────────────
            // getBoundingClientRect()를 onUpdate 안에서 매번 호출하면
            // 브라우저가 매 프레임 layout reflow를 강제로 실행해서 버벅임 발생.
            // 애니메이션 시작 전에 상대 좌표를 한 번만 계산해 캐싱한다.
            ...
            // clipPath 업데이트: 캐싱된 값만 사용 → DOM 읽기 없음
            ...
            tl = gsap.timeline({
                scrollTrigger: {
                    ...
                    // 리사이즈 시 캐시 재계산
                    onRefresh() {
                        leftCache.top = getRelativeRect(leftImg).top;
                        leftCache.bottom = getRelativeRect(leftImg).bottom;
                        leftCache.height = getRelativeRect(leftImg).height;
                        rightCache.top = getRelativeRect(rightImg).top;
                        rightCache.bottom = getRelativeRect(rightImg).bottom;
                        rightCache.height = getRelativeRect(rightImg).height;
                    },
                },
            });
            ...
            tl.fromTo(svgRef.current,
                { scale: 1, transformOrigin: 'bottom left', force3D: true },
                { scale: CONTAINER_SCALE, transformOrigin: 'bottom left', duration: 0.85, ease: 'power2.inOut', force3D: true },
                0
            );

            // ③ 각 글자 stagger
            const step = 0.85 / LETTER_PATHS.length;
            letters.forEach((el, i) => {
                tl.fromTo(el,
                    { scale: 0, transformOrigin: LETTER_ORIGINS[i], force3D: true },
                    { scale: 1, transformOrigin: LETTER_ORIGINS[i], duration: step, ease: 'power2.out', force3D: true },
                    i * step
                );
            });
        });
